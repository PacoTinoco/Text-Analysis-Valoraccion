...
function AISummaryPanel({ fileId, department, label, onSummaryReady }: { fileId: string; department?: string; label: string; onSummaryReady?: (text: string) => void }) {
  ...
  const generate = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(apiUrl("/api/v1/ai-summary"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ file_id: fileId, department }),
      });
      if (!res.ok) { const err = await res.json(); throw new Error(err.detail || "Error al generar resumen"); }
    ...
    return (
      <div className="card p-6 border-2 border-dashed border-purple-200 bg-gradient-to-br from-purple-50 to-blue-50">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-xl bg-purple-100 flex items-center justify-center"><span className="text-xl">🤖</span></div>
          <div><h3 className="font-semibold text-slate-900">Resumen con IA</h3><p className="text-xs text-slate-500">{label}</p></div>
        </div>
        <p className="text-sm text-slate-600 mb-4">La IA analizará los resultados y generará un reporte con hallazgos, fortalezas y recomendaciones.</p>
        <button onClick={generate} className="btn-primary flex items-center gap-2"><span>✨</span> Generar resumen inteligente</button>
      </div>
    );
  }
  if (loading) {
    return (
      <div className="card p-8 text-center">
        <div className="w-12 h-12 rounded-xl bg-purple-100 flex items-center justify-center mx-auto mb-4 animate-pulse"><span className="text-2xl">🤖</span></div>
        <p className="text-slate-700 font-medium">Generando resumen inteligente...</p>
        <p className="text-slate-400 text-sm mt-1">Esto puede tomar 30-90 segundos.</p>
      </div>
    );
  }
  if (error) {
    return (
      <div className="card p-6 border border-red-200 bg-red-50">
        <div className="flex items-center gap-2 mb-2"><span>⚠️</span><h3 className="font-semibold text-red-800">Error</h3></div>
        <p className="text-sm text-red-700 mb-3">{error}</p>
        <button onClick={generate} className="text-sm text-red-600 hover:text-red-800 font-medium underline">Reintentar</button>
      </div>
    );
  }
  return (
    <div className="card overflow-hidden">
      <div className="bg-gradient-to-r from-purple-600 to-blue-600 px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2"><span className="text-white text-lg">🤖</span><h3 className="font-semibold text-white">Resumen generado por IA</h3></div>
        <button onClick={generate} className="text-white/80 hover:text-white text-xs font-medium">🔄 Regenerar</button>
      </div>
...
function MarkdownRenderer({ content }: { content: string }) {
  ...
  const flushList = (key: string) => {
    if (listItems.length > 0) {
      elements.push(<ul key={key} className="list-disc list-inside space-y-1 mb-4 text-slate-700 text-sm">{listItems.map((item, i) => <li key={i} className="leading-relaxed" dangerouslySetInnerHTML={{ __html: item }} />)}</ul>);
      listItems = [];
    }
  };
  lines.forEach((line, i) => {
    ...
    else if (t.startsWith("---")) { flushList("l" + i); elements.push(<hr key={i} className="my-4 border-slate-200" />); }
...
function fmtInline(text: string): string {
  return text.replace(/\*\*(.+?)\*\*/g, '<strong class="font-semibold text-slate-900">$1</strong>').replace(/\*(.+?)\*/g, "<em>$1</em>");
}

function DrilldownModal({ query, fileId, responseColumn, filters, department, onClose }: {
  query: string; fileId: string; responseColumn: string; filters: Record<string, string[]>; department?: string; onClose: () => void;
}) {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center pt-20 px-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[75vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="p-5 border-b border-slate-200 flex items-center justify-between flex-shrink-0">
          <div><h2 className="font-semibold text-slate-900">Respuestas que contienen: &quot;{query}&quot;</h2>
            {data && <p className="text-sm text-slate-500 mt-0.5">{data.total_matches} encontradas{department ? " en " + department : ""}</p>}</div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 text-xl font-bold">✕</button>
        </div>
        {data && !loading && (
          <div className="px-5 py-3 border-b border-slate-100 flex gap-4 flex-shrink-0">
            <MiniSent label="Positivo" count={data.sentiment.positivo} total={data.total_matches} color="text-emerald-600" />
            <MiniSent label="Neutro" count={data.sentiment.neutro} total={data.total_matches} color="text-slate-500" />
            <MiniSent label="Negativo" count={data.sentiment.negativo} total={data.total_matches} color="text-red-600" />
          </div>
        )}
        <div className="flex-1 overflow-y-auto p-5 space-y-2">
          {loading && <p className="text-center text-slate-400 py-8">Buscando...</p>}
          {data?.responses.map((r, i) => <ExpandableResponse key={i} text={r.text} variant={r.sentiment === "positivo" ? "positive" : r.sentiment === "negativo" ? "negative" : "neutral"} previewLength={400} />)}
          {data && data.responses.length === 0 && <p className="text-center text-slate-400 py-8">No se encontraron</p>}
        </div>
      </div>
    </div>
  );
...
function MiniSent({ label, count, total, color }: { label: string; count: number; total: number; color: string }) {
  const pct = total > 0 ? Math.round((count / total) * 100) : 0;
  return <div className="text-sm"><span className="text-slate-500">{label}: </span><span className={"font-medium " + color}>{pct}% ({count})</span></div>;
}
...
function ResultsDashboard({ data, config, onReset }: { data: AnalysisResult; config: AnalysisConfig; onReset: () => void }) {
  ...
  const sentimentPie = [
    { name: "Positivo", value: general.sentiment.positivo },
    { name: "Neutro", value: general.sentiment.neutro },
    { name: "Negativo", value: general.sentiment.negativo },
  ];
  const wordChartData = general.top_words.slice(0, 12).map((w) => ({ name: w.word, cantidad: w.count }));
  const deptCompareData = deptNames.map((name) => {
    const d = by_group![name]; const t = d.summary.valid_responses || 1;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="card p-5"><h3 className="font-semibold text-slate-900 mb-4">Distribución de sentimiento</h3>
          <ResponsiveContainer width="100%" height={250}><PieChart><Pie data={sentimentPie} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={3} dataKey="value" label={({ name, percent }) => name + " " + (percent * 100).toFixed(0) + "%"}>{sentimentPie.map((_, i) => <Cell key={i} fill={COLORS[i]} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer>
        </div>
        <div className="card p-5"><h3 className="font-semibold text-slate-900 mb-4">Palabras más frecuentes</h3>
          <ResponsiveContainer width="100%" height={250}><BarChart data={wordChartData} layout="vertical" margin={{ left: 60 }}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" /><XAxis type="number" tick={{ fontSize: 12 }} /><YAxis type="category" dataKey="name" tick={{ fontSize: 12 }} width={55} /><Tooltip /><Bar dataKey="cantidad" fill="#3b82f6" radius={[0, 4, 4, 0]} /></BarChart></ResponsiveContainer>
        </div>
      </div>

      <div className="flex gap-1 bg-slate-100 p-1 rounded-lg w-fit">
        {(["general", ...(by_group ? ["departments", "compare"] : []), "ai"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t as any)} className={"px-4 py-2 text-sm font-medium rounded-md transition-colors " + (tab === t ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700")}>
            {t === "general" ? "📋 General" : t === "departments" ? "🏢 Por departamento" : t === "compare" ? "📊 Comparativa" : "🤖 Resumen IA"}
          </button>
        ))}
      </div>
      ...
      {tab === "general" && (<>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <HighlightsSection title="✨ Respuestas positivas" subtitle="Mayor contenido positivo" items={general.highlights.positive} variant="positive" />
      ...
      {tab === "ai" && (<div className="space-y-6">
        <AISummaryPanel fileId={config.file_id} label="Resumen ejecutivo de todas las evaluaciones" onSummaryReady={setAiSummaryText} />
        {by_group && deptNames.length > 0 && (<div className="card p-5"><h3 className="font-semibold text-slate-900 mb-1">Resúmenes por departamento</h3><p className="text-xs text-slate-400 mb-4">Genera un reporte para cada departamento</p><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{deptNames.map((name) => <DeptAICard key={name} name={name} fileId={config.file_id} data={by_group[name]} />)}</div></div>)}
      </div>)}
    ...
    <div className="space-y-4">
      <div className="card p-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-emerald-50 rounded-lg p-3 text-center"><p className="text-2xl font-bold text-emerald-600">{Math.round((data.sentiment.positivo / total) * 100)}%</p><p className="text-xs text-emerald-700 mt-0.5">Positivo ({data.sentiment.positivo.toLocaleString()})</p></div>
          <div className="bg-slate-100 rounded-lg p-3 text-center"><p className="text-2xl font-bold text-slate-600">{Math.round((data.sentiment.neutro / total) * 100)}%</p><p className="text-xs text-slate-600 mt-0.5">Neutro ({data.sentiment.neutro.toLocaleString()})</p></div>
          <div className="bg-red-50 rounded-lg p-3 text-center"><p className="text-2xl font-bold text-red-600">{Math.round((data.sentiment.negativo / total) * 100)}%</p><p className="text-xs text-red-700 mt-0.5">Negativo ({data.sentiment.negativo.toLocaleString()})</p></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="card p-5"><h4 className="font-medium text-slate-900 mb-1 text-sm">Bigramas</h4><p className="text-xs text-slate-400 mb-2">Clic para ver respuestas</p><div className="space-y-1.5">{data.top_phrases.slice(0, 8).map((item, i) => <ClickableBar key={i} label={item.phrase} count={item.count} max={data.top_phrases[0]?.count || 1} onClick={() => onDrilldown(item.phrase, name)} />)}</div></div>
        <div className="card p-5"><h4 className="font-medium text-slate-900 mb-1 text-sm">Trigramas</h4><p className="text-xs text-slate-400 mb-2">Clic para ver respuestas</p><div className="space-y-1.5">{data.top_trigrams.slice(0, 8).map((item, i) => <ClickableBar key={i} label={item.phrase} count={item.count} max={data.top_trigrams[0]?.count || 1} onClick={() => onDrilldown(item.phrase, name)} />)}</div></div>
      </div>
      {data.top_names.length > 0 && <div className="card p-5"><h4 className="font-medium text-slate-900 mb-1 text-sm">👤 Nombres mencionados</h4><p className="text-xs text-slate-400 mb-2">Clic para ver contexto</p><div className="flex flex-wrap gap-2">{data.top_names.slice(0, 15).map((item, i) => <button key={i} onClick={() => onDrilldown(item.name, name)} className="px-3 py-1.5 bg-purple-50 text-purple-700 rounded-full text-xs font-medium border border-purple-200 hover:bg-purple-100 transition-colors cursor-pointer">{item.name} ({item.count})</button>)}</div></div>}
      <AISummaryPanel fileId={fileId} department={name} label={"Reporte IA para " + name} />
      {data.highlights.positive.length > 0 && <HighlightsSection title="✨ Destacadas" subtitle="Respuestas positivas" items={data.highlights.positive.slice(0, 8)} variant="positive" />}
      {data.highlights.negative.length > 0 && <HighlightsSection title="⚠️ Áreas de oportunidad" subtitle="Respuestas críticas" items={data.highlights.negative.slice(0, 8)} variant="negative" />}
      {data.suggestions.length > 0 && <HighlightsSection title={"💡 Sugerencias (" + data.suggestions.length + ")"} subtitle="Propuestas de mejora" items={data.suggestions.slice(0, 8)} variant="suggestion" />}
    </div>
  );
}
...
function HighlightsSection({ title, subtitle, items, variant }: { title: string; subtitle: string; items: string[]; variant: "positive" | "negative" | "suggestion" }) {
  ...
  return (
    <div className="card p-5"><h3 className="font-semibold text-slate-900 mb-0.5">{title}</h3><p className="text-xs text-slate-400 mb-3">{subtitle}</p>
      <div className="space-y-2">{visible.map((resp, i) => <ExpandableResponse key={i} text={resp} variant={variant} />)}{items.length === 0 && <p className="text-sm text-slate-400 italic">No se encontraron</p>}</div>
      {items.length > 3 && <button onClick={() => setShowAll(!showAll)} className="mt-3 text-sm text-blue-600 hover:text-blue-800 font-medium">{showAll ? "▲ Mostrar menos" : "▼ Ver todas (" + items.length + ")"}</button>}
    </div>
  );
}

function ClickableBar({ label, count, max, onClick, color = "bg-blue-500" }: { label: string; count: number; max: number; onClick: () => void; color?: string }) {
...
function MetricCard({ label, value, icon }: { label: string; value: string; icon: string }) {
  return (<div className="card p-4"><div className="flex items-center gap-2 mb-1"><span className="text-base">{icon}</span><p className="text-xs font-medium text-slate-500">{label}</p></div><p className="text-xl font-bold text-slate-900">{value}</p></div>);
}

function StepBadge({ n, label, active, done }: { n: number; label: string; active: boolean; done: boolean }) {
  return (<div className="flex items-center gap-2"><div className={"w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold " + (done ? "bg-emerald-500 text-white" : active ? "bg-blue-600 text-white" : "bg-slate-200 text-slate-500")}>{done ? "✓" : n}</div><span className={"text-sm font-medium " + (active || done ? "text-slate-900" : "text-slate-400")}>{label}</span></div>);
}